---
name: system-design
description: Use when creating, reviewing, or improving polished HTML/CSS/JS design artifacts, including prototypes, slide-like HTML decks, visual explorations, interaction demos, design-system extractions, and frontend mockups. This skill emphasizes designer judgment, strong visual hierarchy, accessibility, intentional aesthetics, real interaction states, and avoiding generic AI-looking design.
metadata:
  short-description: Build polished HTML design artifacts
---
...
You have a library of skills in `~/.agents/skills/system-design/references/` — each is a phased procedure with explicit checks and fixes. When a user request matches a skill description, **read that file** (e.g. `~/.agents/skills/system-design/references/make-a-deck.md`) and follow it. There is no `Skill` tool in Codex; skills are reference documents you load with a file read.

## Production skills (build something)

- **`~/.agents/skills/system-design/references/discovery-questions.md`** — Run a structured kickoff question round at the start of new or ambiguous work. Use first when you don't have enough context to design.
- **`~/.agents/skills/system-design/references/frontend-aesthetic-direction.md`** — When there's no existing brand or design system, commit to a specific aesthetic (typography, color, density, mood, component style) before drawing hi-fi.
- **`~/.agents/skills/system-design/references/wireframe.md`** — Produce 3+ low-fidelity variations to explore a flow or layout before committing to hi-fi. Greyscale, no brand color, disposable.
- **`~/.agents/skills/system-design/references/make-a-deck.md`** — Build a slide presentation in HTML with fixed-size scaling, layout system, and the deck-shell starter component.
- **`~/.agents/skills/system-design/references/make-a-prototype.md`** — Build a working interactive clickable prototype with real state, navigation, validation, loading states, and feedback.
- **`~/.agents/skills/system-design/references/make-tweakable.md`** — Add a floating tweak panel to a finished design so the user can adjust colors, fonts, copy, or layout variants live.
- **`~/.agents/skills/system-design/references/generate-variations.md`** — Produce 3+ distinct design variations across substantive axes (layout, hierarchy, interaction, tone) — basic to bold — in a single file.

## System skills (extract structure)

- **`~/.agents/skills/system-design/references/design-system-extract.md`** — Pull design tokens (color, type, spacing, radii, shadow) from a brand, codebase, or screenshots and emit a tokens file.
- **`~/.agents/skills/system-design/references/component-extract.md`** — Walk a design and identify reusable components, variants, and states; emit a component inventory.

## Review skills (audit and fix)

- **`~/.agents/skills/system-design/references/accessibility-audit.md`** — Comprehensive accessibility review (contrast, semantic HTML, keyboard nav, motion, forms) followed by auto-fix.
- **`~/.agents/skills/system-design/references/ai-slop-check.md`** — Single-pass review for AI-template tropes (gratuitous gradients, emoji decoration, rounded+left-border cards, hand-drawn SVG, overused fonts) with auto-fix.
- **`~/.agents/skills/system-design/references/hierarchy-rhythm-review.md`** — Check visual hierarchy (size/weight/color) and rhythm (spacing scale, repetition with strategic break) and flag random values.
- **`~/.agents/skills/system-design/references/interaction-states-pass.md`** — Verify every interactive element has hover, active, disabled, and focus states plus appropriate transitions; add what's missing.
- **`~/.agents/skills/system-design/references/polish-pass.md`** — End-of-design quality gate. Runs accessibility-audit, ai-slop-check, interaction-states-pass, and hierarchy-rhythm-review sequentially, then fixes issues.

## When to invoke which

- User asks for something new and ambiguous → `~/.agents/skills/system-design/references/discovery-questions.md` first
- No existing brand and the user wants hi-fi → `~/.agents/skills/system-design/references/frontend-aesthetic-direction.md` before drawing
- "Show me a few options" / "explore this" → `~/.agents/skills/system-design/references/wireframe.md` (low-fi) or `~/.agents/skills/system-design/references/generate-variations.md` (hi-fi)
- "Make a deck" / "build a presentation" → `~/.agents/skills/system-design/references/make-a-deck.md`
- "Make it interactive" / "build a prototype" → `~/.agents/skills/system-design/references/make-a-prototype.md`
- "Let me play with options" / "make this adjustable" → `~/.agents/skills/system-design/references/make-tweakable.md`
- "Extract tokens from this" / "give me a tokens file" → `~/.agents/skills/system-design/references/design-system-extract.md`
- "Identify reusable parts" / "build a component library" → `~/.agents/skills/system-design/references/component-extract.md`
- "Run an accessibility check" → `~/.agents/skills/system-design/references/accessibility-audit.md`
- "This looks AI-generated" / "remove the slop" → `~/.agents/skills/system-design/references/ai-slop-check.md`
- "Check the hierarchy" / "the spacing feels off" → `~/.agents/skills/system-design/references/hierarchy-rhythm-review.md`
- "Verify the states" / "every button has hover/focus" → `~/.agents/skills/system-design/references/interaction-states-pass.md`
- Before delivery / before shipping → suggest `~/.agents/skills/system-design/references/polish-pass.md` as a final gate

```mermaid
flowchart TD
    A["Start Design Task"] --> B{"Existing brand or design system?"}

    B -- "No / Greenfield" --> C["discovery-questions"]
    C --> D["frontend-aesthetic-direction"]
    D --> E["wireframe"]
    E --> F["make-a-prototype"]
    F --> G["polish-pass"]

    B -- "Yes / Brand-aware" --> H["design-system-extract"]
    H --> I["generate-variations"]
    I --> J["make-tweakable"]
    J --> G

    G --> K["Ready to deliver"]
