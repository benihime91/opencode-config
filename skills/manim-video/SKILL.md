---
name: manim-video
description: Use when creating Manim-based technical explainer animations for systems, graphs, workflows, metrics, or product concepts.
---
...
- an external video-editing workflow only when final assembly or polish adds value
- an additional compositing workflow only when the final package needs captions, UI overlays, or extra motion layers
...
7. Hand off to a broader video workflow only if it adds value.
...
```bash
uvx manim -ql assets/network_graph_scene.py NetworkGraphExplainer
```
