...
#
# When CWD_SKILLS_DIR is omitted, defaults to $PWD/skills so the
# script always picks up project-level skills without relying on the caller.
#
# Environment:
#   RULES_DISTILL_GLOBAL_DIR   Override ~/.config/opencode/skills (for testing only;
...
CWD_SKILLS_DIR="${RULES_DISTILL_PROJECT_DIR:-${1:-$PWD/skills}}"
# Validate CWD_SKILLS_DIR looks like an OpenCode skills path (defense-in-depth).
...
# Merge global + project skills into one array, deduplicated by path.
